...
collection_and_queries[2] = ('customers', {"name": {'$regex': 'Natalie \w*'}}, {"username", "name", "address"})

### 3. Find all accounts with a 'products' array containing 'Commodity' -- return the '_id' and 'account_id'
collection_and_queries[3] = ('accounts', {"products": "Commodity"}, {"_id", "account_id"})
...
collection_and_queries[4] = ('accounts', {"$or": [{"products": ["Commodity", "InvestmentStock"]}, {"limit": {"$lte": 9000}}]})
...
collection_and_queries[5] = ('accounts', {"$and": [{"products": ["Commodity", "InvestmentStock"]}, {"limit": {"$lte": 9000}}]})
...
collection_and_queries[8] = ('customers', [{"$match": {"name": {'$regex': '^[A-G]'}}},
                                           {"$group": {"_id": {"$month": "$birthdate"}, "totalCount": {"$sum": 1}}}])
...
###
collection_and_queries[9] = ('transactions', [{'$unwind': '$transactions'},
...
collection_and_queries[10] = ('transactions', [{'$unwind': '$transactions'},
...
collection_and_queries[11] = ('customers', [{'$unwind': '$accounts'},
                                            {"$lookup": {"from": "accounts", "localField": "accounts", "foreignField": "account_id", "as":"account_info"}},
                                            {"$match": {"account_info.0.products": "InvestmentFund"}},
...
collection_and_queries[12] = ('accounts', [{"$match": {"products": {"$size": 3}}},
                                           {"$lookup": {"from": "transactions", "localField": "account_id", "foreignField": "account_id", "as":"account_info"}},
                                           {"$match": {"account_info.0.transaction_count": {"$lte": 10}}},
                                            {'$unwind': '$account_info'},
                                            {"$addFields": {"transaction_count": "$account_info.transaction_count"}},
