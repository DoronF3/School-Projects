...
class GroupedMapInPandasTests(ReusedSQLTestCase):
    ...
    def test_supported_types(self):
        ...
        expected1 = df.toPandas().groupby('id').apply(udf1.t8_helper).reset_index(drop=True)
    ...
    def test_array_type_correct(self):
        ...
        expected = df.toPandas().groupby('id').apply(udf.t8_helper).reset_index(drop=True)
    ...
    def test_coerce(self):
        ...
        result = df.groupby('id').apply(foo).sort('id').toPandas()
        expected = df.toPandas().groupby('id').apply(foo.t8_helper).reset_index(drop=True)
    ...
    def test_datatype_string(self):
        ...
        expected = df.toPandas().groupby('id').apply(foo_udf.t8_helper).reset_index(drop=True)
        assert_frame_equal(expected, result)
    ...
    def test_udf_with_key(self):
        ...
        expected1 = pdf.groupby('id', as_index=False)\
            .apply(lambda x: udf1.t8_helper((x.id.iloc[0],), x))\
        ...
        expected2 = pdf.groupby(pdf.id % 2, as_index=False)\
            .apply(lambda x: udf1.t8_helper((x.id.iloc[0] % 2,), x))\
            .sort_values(['id', 'v']).reset_index(drop=True)
        assert_frame_equal(expected2, result2)
        ...
        expected3 = pdf.groupby([pdf.id, pdf.v % 2], as_index=False)\
            .apply(lambda x: udf2.t8_helper((x.id.iloc[0], (x.v % 2).iloc[0],), x))\
            .sort_values(['id', 'v']).reset_index(drop=True)
        assert_frame_equal(expected3, result3)

        # Test empty groupby
        result4 = df.groupby().apply(udf3).sort('id', 'v').toPandas()
        expected4 = udf3.t8_helper((), pdf)
